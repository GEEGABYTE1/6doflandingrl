"""Modular disturbance models for the Phase 1 simulator."""
...
class WindModel:
    """Constant wind plus optional deterministic sinusoidal gust in inertial axes."""
...
class ThrustMisalignmentModel:
    ...
    def angles(self, time_s: float) -> Array:
        """Return ``[pitch_bias, yaw_bias]`` in radians."""
        del time_s
...
class SensorNoiseModel:
    """Gaussian sensor noise for future controller observation interfaces."""
    ...
    def __post_init__(self) -> None:
        """Create the deterministic random generator."""
        self.rng = np.random.default_rng(self.seed)

    def noisy_state(self, state: Array) -> Array:
        """Return a noisy copy of the simulator state vector."""
...
class DisturbanceModel:
    """Container for separable wind, thrust misalignment, and sensor noise."""
